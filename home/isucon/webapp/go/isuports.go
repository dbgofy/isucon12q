...
func Run() {
	...
	pprofGroup := e.Group("/api/debug/pprof")
...
func retrievePlayer(ctx context.Context, tenantDB dbOrTx, tenantId int64, id string) (*PlayerRow, error) {
	...
	if err := tenantDB.GetContext(ctx, &p, "SELECT * FROM player WHERE tenant_id = ? AND id = ?", tenantId, id); err != nil {
		return nil, fmt.Errorf("error Select player: tenantId=%d, id=%s, %w", tenantId, id, err)
	}
	return &p, nil
}
...
// 大会ごとの課金レポートを計算する
func billingReportByCompetition(ctx context.Context, tenantDB dbOrTx, tenantID int64, competitonID string, vhs []VisitHistorySummaryRow) (*BillingReport, error) {
	...
	if vhs == nil {
		if err := adminDB.SelectContext(
			ctx,
			&vhs,
			"SELECT player_id, MIN(created_at), tenant_id, competition_id AS min_created_at FROM visit_history WHERE tenant_id = ? AND competition_id = ? GROUP BY player_id",
			tenantID,
			comp.ID,
	...
	if len(vhs) == 0 {
		return nil, fmt.Errorf("error Select visit_history: tenantID=%d, competitionID=%s, %w", tenantID, comp.ID, errors.New("vhs is empty"))
	}
